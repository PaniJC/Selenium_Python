...
class SandboxPage(BasePage):
    ...
    def get_sports_dropdown_options(self):
    ...
    def get_cell_value(self, row, col):
        cell_xpath = f"(//table[@class='table table-striped table-bordered table-hover']/tbody/tr)[{row}]/td[{col}]"
        
        cell = self.wait_for_element((By.XPATH, cell_xpath))
        return cell.text if cell else None
    
    def get_cell_value_static(self, row, col):
        cell_xpath = f"(//h2[normalize-space()='Tabla estática']/following-sibling::table/tbody/tr)[{row}]/td[{col}]"
